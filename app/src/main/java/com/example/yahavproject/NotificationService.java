        ...
        String email_service = sharedPref.getString(getString(R.string.saved_email), "");
            ...
            Runnable runnable = new Runnable() {
                ...
                public void run() {
                    ...
                    //Toast.makeText(NotificationService.this, currentDay+" "+currentTime, Toast.LENGTH_SHORT).show();

/*                    getCurrentDay("Sunday", "Sun", email_service);
                    getCurrentDay("Monday", "Mon", email_service);
                    getCurrentDay("Tuesday", "Tue", email_service);
                    getCurrentDay("Wednesday", "Wed", email_service);
                    getCurrentDay("Thursday", "Thu", email_service);
                    getCurrentDay("Friday", "Fri", email_service);
                    getCurrentDay("Saturday", "Sat", email_service);*/
...
/*                    q = myRef.child("my app users").child(email_service)
                    ...
                    ValueEventListener valueEventListener = new ValueEventListener() {    //start
                        @Override
                                ...
                                for (DataSnapshot dst : dataSnapshot.getChildren()) {
                                    ...
                                    if (noteItem.noteColor != R.color.done_note) {
                                        if ((noteItem.note_time).equals(currentTime)) {
                                            thisNote = noteItem;
                                            notifyRightNow();
                                            //Toast.makeText(NotificationService.this, noteItem.note_title, Toast.LENGTH_SHORT).show();
                                        }
                                    }
                                }
                            }
    ...
    public void getCurrentDay (String dayName, String current, String user) {

        if (currentDay.equals(current)) {
            q = myRef.child("my app users").child(user)
                    .child("days").child(dayName).child("NOTES").orderByValue();
        }
    }
